...
- [ ] If the change touches `--upload`, `--watch`, or `--schedule`: live-tested with `MUNINN_TEST_ALLOW_LIVE_KEY=1` set on a host with a sacrificial key, OR with `--dry-run` baked in.
- [ ] README code blocks updated where applicable; `python scripts/check_readme_examples.py README.md` passes (if the linter is wired in).
- [ ] CHANGELOG.md has an entry for this change.
- [ ] `__version__` in `muninn.py` is bumped if user-visible behavior changed (skip for docs-only / CI-only changes).
