"""Tests for the v2.0.15 wrapper-refresh half of --update.

Background: the raw-download (ZIP install) update path historically
fetched only muninn.py + requirements.txt, so a bug fix that lived in
...
shape as the Heimdall / wigle-to-wdgwars implementations modulo naming.
"""
...
class RefreshWrappersTests(unittest.TestCase):
    ...
    def test_refreshes_only_wrappers_present_on_disk(self) -> None:
        ...
        with self._tree_with(present) as td, \
             mock.patch.object(muninn, "_fetch_raw", return_value=True) as fetch, \
             mock.patch("sys.stderr", new=io.StringIO()):
            muninn._refresh_wrappers(Path(td))
    ...
    def test_one_failed_wrapper_does_not_stop_the_rest(self) -> None:
        ...
        err = io.StringIO()
        with self._tree_with(present) as td, \
             mock.patch.object(muninn, "_fetch_raw", side_effect=fail_run_sh) as fetch, \
    ...
    def test_sh_wrappers_get_exec_bit_on_posix(self) -> None:
        ...
        with self._tree_with(present) as td:
            ...
            with mock.patch.object(muninn, "_fetch_raw", return_value=True), \
                 mock.patch("os.name", "posix"), \
                 mock.patch("os.chmod") as chmod, \
                 mock.patch("sys.stderr", new=io.StringIO()):
                muninn._refresh_wrappers(root)
...
class UpdateFromRawCallsWrapperRefreshTests(unittest.TestCase):
    ...
    def test_already_latest_branch_refreshes_wrappers(self) -> None:
        same_version = f'__version__ = "{muninn.__version__}"\n'
        with tempfile.TemporaryDirectory() as td, \
             self._fake_urlopen_returning(same_version), \
             mock.patch.object(muninn, "_fetch_raw", return_value=True), \
             mock.patch.object(muninn, "_refresh_wrappers") as refresh, \
